...
def plot_high(options, start_date, end_date):
    """
    ...
    """
    d = {'Google' : 'GOOGL',  'Facebook' : 'FB', 'Tesla' : 'TSLA', 'Ford' : 'F', 'GM' : 'GM', 'Microsoft' : 'MSFT', 'Apple' : 'AAPL'}
    fig = go.Figure()
    for opt in options:
    ...
    fig.update_layout(title = "Daily High Price", yaxis_title = 'Stock Prices', xaxis_title = 'Date')
    st.plotly_chart(fig)

def plot_close(options, start_date, end_date):
    """
    Plotting the close prices daily
    ...
    for opt in options:
        df = pd.DataFrame(web.DataReader(d[opt], 'yahoo', start_date, end_date)) 
        fig.add_trace(go.Scatter(x=df.index.get_level_values('Date'), y = df['Close'], mode = 'lines', name = f'{opt}'))
    fig.update_layout(title = "Daily Close Price", yaxis_title = 'Stock Prices', xaxis_title = 'Date')
    st.plotly_chart(fig)
...
def plot_volume_traded(options, start_date, end_date):
    ...
    fig.update_layout(title = f"Daily Volume Traded", yaxis_title = 'Volume', xaxis_title = 'Date')
    st.plotly_chart(fig)
...
def main():
    ...
    option = st.multiselect(
        'Company Name',
        ['Google', 'Facebook', 'Tesla', 'Ford', 'GM', 'Microsoft', 'Apple']
    )

    if option is not None:
        plot_high(option, start_date, end_date)

        plot_close(option, start_date, end_date)
